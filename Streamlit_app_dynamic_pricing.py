...
# Load data (ensure the 'dynamic_pricing.csv' file is in the same directory)
...
def train_model(data):
    ...
    # Use the one-hot encoded columns
...
def predict_price(model, number_of_riders, number_of_drivers, vehicle_type, expected_ride_duration):
    vehicle_type_numeric = 1 if vehicle_type == "Premium" else 0  # Simplified conversion
...
predicted_price = predict_price(model, user_number_of_riders, user_number_of_drivers, user_vehicle_type,
...
# Use the preprocessed data for testing and visualization
x_test = data[["Number_of_Riders", "Number_of_Drivers", "Vehicle_Type_Premium", "Expected_Ride_Duration"]]
y_test = data["adjusted_ride_cost"]
